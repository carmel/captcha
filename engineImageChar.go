...
// 新建一个图片对象.
func newCaptchaImage(width int, height int, bgColor color.RGBA) (cImage *ImageChar, err error) {
...
func (captcha *ImageChar) drawBeeline(point1 point, point2 point, lineColor color.RGBA) {
...
func (captcha *ImageChar) drawText(text string, isSimpleFont bool) error {
	c := freetype.NewContext()
	c.SetDPI(imageStringDpi)
...
func EngineCharCreate(config ConfigCharacter) *ImageChar {
