...
// ImageDigit digits captcha Struct
...
func EngineDigitsCreate(id string, config ConfigDigit) *ImageDigit {
...
func (m *ImageDigit) getRandomPalette() color.Palette {
...
func (m *ImageDigit) calculateSizes(width, height, ncount int) {
...
func (m *ImageDigit) drawHorizLine(fromX, toX, y int, colorIdx uint8) {
	for x := fromX; x <= toX; x++ {
		m.SetColorIndex(x, y, colorIdx)
	}
}

func (m *ImageDigit) drawCircle(x, y, radius int, colorIdx uint8) {
	f := 1 - radius
	dfx := 1
	dfy := -2 * radius
	xo := 0
...
func (m *ImageDigit) fillWithCircles(n, maxradius int) {
	maxx := m.Bounds().Max.X
	maxy := m.Bounds().Max.Y
	for range n {
...
func (m *ImageDigit) strikeThrough() {
	maxx := m.Bounds().Max.X
	maxy := m.Bounds().Max.Y
...
// 写入数字 数字byte
func (m *ImageDigit) drawDigit(digit []byte, x, y int) {
...
func (m *ImageDigit) distort(amplude float64, period float64) {
	w := m.Bounds().Max.X
	h := m.Bounds().Max.Y
...
func (m *ImageDigit) randomBrightness(c color.RGBA, max uint8) color.RGBA {
...
func (m *ImageDigit) BinaryEncodeing() []byte {
...
func (m *ImageDigit) WriteTo(w io.Writer) (int64, error) {
