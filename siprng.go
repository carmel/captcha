...
func (r *siprng) Seed(seed [16]byte) {
	k0 := uint64(seed[0]) | uint64(seed[1])<<8 | uint64(seed[2])<<16 |
		uint64(seed[3])<<24 | uint64(seed[4])<<32 | uint64(seed[5])<<40 |
		uint64(seed[6])<<48 | uint64(seed[7])<<56
	k1 := uint64(seed[8]) | uint64(seed[9])<<8 | uint64(seed[10])<<16 |
		uint64(seed[11])<<24 | uint64(seed[12])<<32 | uint64(seed[13])<<40 |
		uint64(seed[14])<<48 | uint64(seed[15])<<56
	r.v0 = k0 ^ 0x736f6d6570736575
	r.v1 = k1 ^ 0x646f72616e646f6d
	r.v2 = k0 ^ 0x6c7967656e657261
	r.v3 = k1 ^ 0x7465646279746573
}

func (r *siprng) Uint64() uint64 {
	r.v3++
	for range sipRound {
		r.v0 += r.v1
		r.v1 = rotl(r.v1, 13)
...
func (r *siprng) Float64() float64 {
	return float64(r.Uint64()>>11) / (1 << 53)
}

func (r *siprng) Intn(n int) int {
	return int(r.Uint64() % uint64(n))
}

func rotl(x uint64, b int) uint64 {
	return (x << b) | (x >> (64 - b))
}
